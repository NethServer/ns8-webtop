...
container=$(buildah from docker.io/library/httpd:2.4.67)
...
container=$(buildah from docker.io/library/php:7.3-fpm-alpine)
buildah copy --from=docker.io/mlocati/php-extension-installer:2.10.20 ${container} /usr/bin/install-php-extensions /usr/local/bin/
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-webtop; then
    ...
    buildah from --name nodebuilder-webtop -v "${PWD}:/usr/src:Z" docker.io/library/node:24.15.0-slim
fi
