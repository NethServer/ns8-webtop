...
container=$(buildah from docker.io/library/tomcat:9.0.118-jre8)
...
container=$(buildah from docker.io/library/php:7.3-fpm-alpine)
buildah copy --from=docker.io/mlocati/php-extension-installer:2.11.11 ${container} /usr/bin/install-php-extensions /usr/local/bin/
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-webtop; then
    ...
    buildah from --name nodebuilder-webtop -v "${PWD}:/usr/src:Z" docker.io/library/node:24.16.0-slim
fi
