...
container=$(buildah from docker.io/library/postgres:17.10)
...
container=$(buildah from docker.io/library/httpd:2.4.68)
...
container=$(buildah from docker.io/library/php:7.3-fpm-alpine)
buildah copy --from=docker.io/mlocati/php-extension-installer:2.11.12 ${container} /usr/bin/install-php-extensions /usr/local/bin/
...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-webtop; then
    ...
    buildah from --name nodebuilder-webtop -v "${PWD}:/usr/src:Z" docker.io/library/node:24.18.1-slim
fi

echo "Build static UI files with node..."
buildah run --env="NODE_OPTIONS=--openssl-legacy-provider" nodebuilder-webtop sh -c "cd /usr/src/ui && corepack enable && yarn install && yarn build"
